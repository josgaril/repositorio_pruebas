...
function init() {
    ...
    //pintarListado(personas.alumnos(el=> el.genero == "H"));
    const hombres = alumnos.filter(el=> el.genero == "H");
    console.debug(hombres);
    pintarListado(hombres);
}
...
function pintarListado(arrayPersonas){
    ...
    arrayPersonas.forEach( el => listado.innerHTML += `<li><img src="${el.avatar}" alt="avatar">${el.nombre}</li>` );
